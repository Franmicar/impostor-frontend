...
### Versión 1.4.5
- **Optimización de UX Premium**: Refactorización del layout global (`app.ts` y `styles.css`) inyectando dinámicamente la clase `.premium-active` para suprimir el margen inferior muerto (`padding-bottom: 96px;`) que estaba reservado al banner nativo de Google AdMob. Esto asegura que todos los flujos de juego ("Revelar rol", "Ruleta", "Votar", "Resultados") re-centren su contenido y aprovechen todo el aspecto físico de la pantalla tras comprar la suscripción.
- **Estandarización de Footers Flotantes**: Homogeneización de los botones fijos inferiores de todo el ecosistema de ajustes (Modos, Tipos, Jugadores, Paquetes) abrazando el estilo cristalizado (`bg-gradient-to-t...`) implementado en el menú base, y desanclándolos de los recuadros sólidos clásicos.
- **Calibración de Zonas de Impacto Publicitario**: Corrección del cálculo de espaciado en la interfaz aislando el padding horizontal (`px-4`) del margen inferior reservado. Se ha inyectado consciencia del entorno a las pantallas de Ruleta (`play.ts`) y Resultados (`results.ts`) para que su centrado maestro (`justify-center`) bascule la masa visual descontando matemáticamente la franja del banner, garantizando una visibilidad óptima en móviles compactos.
