  ...
  <div class="min-h-dvh bg-transparent text-textPrimary flex flex-col overflow-hidden relative">
   ...
   @if (!engine.gameStarted()) {
    ...
    <div class="z-10 w-full max-w-lg mb-4 flex flex-col items-center gap-2 mt-2">
      <div class="flex items-start justify-between w-full">
        ...
        <div class="flex flex-col items-end">
          <span class="text-xs uppercase tracking-widest text-textMuted font-bold mb-1">{{ 'DRAW.TIME_LEFT' | translate }}</span>
          <div class="relative w-14 h-14 flex items-center justify-center bg-glass border border-glass-border rounded-full shadow-[0_0_15px_rgb(var(--color-primary)/0.4)]">
            <span class="text-2xl font-black" [class.text-red-500]="timeLeft() <= 3" [class.text-textPrimary]="timeLeft() > 3" [class.animate-pulse]="timeLeft() <= 3">
              {{ timeLeft() }}
            </span>
            <!-- Circle Progress -->
      ...
      <div class="w-full h-1.5 bg-glass border border-glass-border rounded-full overflow-hidden mt-2">
    ...
    <div class="z-10 w-full max-w-lg flex-1 flex flex-row gap-4 mb-6 relative">
      ...
      @if(!isDrawingPhaseFinished()) {
       <div class="flex flex-col gap-3 z-30 shrink-0 mt-4">
         <button (click)="undo()" 
             ...
             class="w-12 h-12 rounded-full bg-glass shadow-md flex items-center justify-center text-textPrimary active:scale-95 transition-all border border-glass-border disabled:cursor-not-allowed">
         ...
         <div class="relative">
           <button (click)="isPaletteOpen = !isPaletteOpen" class="w-12 h-12 rounded-full shadow-md flex items-center justify-center border-2 border-glass-border active:scale-95 transition-transform" [style.backgroundColor]="currentColor">
           </button>
           
           <!-- Color Palette Dropdown -->
           @if(isPaletteOpen) {
             <div class="absolute top-14 left-0 bg-glass backdrop-blur-md border border-glass-border rounded-2xl p-2 shadow-xl flex flex-col gap-2 z-40">
      ...
      <!-- Interacive Canvas -->
      <div class="flex-1 bg-white rounded-3xl shadow-[0_0_30px_rgba(0,0,0,0.5)] overflow-hidden relative border-[4px] border-glass-border min-h-[300px]">
        <canvas #drawCanvas 
        ...
        @if (!isCurrentPlayerTurn()) {
          <div class="absolute inset-0 bg-glass backdrop-blur-md flex flex-col items-center justify-center px-6 text-center z-20">
            ...
            @if (isDrawingPhaseFinished()) {
              <h3 class="text-2xl font-bold text-textPrimary mb-2">{{ 'DRAW.ALL_DONE' | translate }}</h3>
              <p class="text-textMuted">{{ 'DRAW.ALL_DONE_DESC' | translate }}</p>
            } @else {
