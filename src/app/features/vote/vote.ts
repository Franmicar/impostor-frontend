 ...
 template: `
  <div class="min-h-dvh bg-transparent text-textPrimary flex flex-col relative w-full">
   ...
   @if (!engine.gameStarted()) {
    <div class="flex-1 flex flex-col items-center justify-center gap-4 h-full my-auto px-6 text-center">
          ...
          @if (isOnline()) {
            <div class="absolute bottom-0 right-0 w-5 h-5 rounded-full flex items-center justify-center border shadow-md z-10"
              ...
              @if (isPlayerMuted(player.id)) {
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3 h-3 text-white"><path stroke-linecap="round" stroke-linejoin="round" d="M12 18.75a6 6 0 0 0 6-6v-1.5m-6 7.5a6 6 0 0 1-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 0 1-3-3V4.5a3 3 0 1 1 6 0v8.25a3 3 0 0 1-3 3Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M3 3l18 18" /></svg>
              } @else {
...
export class Vote implements OnInit, OnDestroy {
 ...
 eliminate() {
  ...
  if (!gameEnded && this.engine.currentSettings()?.modeId === 'fast') {
   ...
   this.timer.stop();
   this.router.navigate(['/results'], { queryParams: { winner: 'impostors' }, state: { intentional: true }, replaceUrl: true });
   return;
  }
 ...
 checkWinConditions(): boolean {
  ...
  if (modeId === 'chaos') {
   if (originalImpostors === 0) {
    if (eliminations >= 1) {
     this.timer.stop();
     this.router.navigate(['/results'], { queryParams: { winner: 'town' }, state: { intentional: true }, replaceUrl: true });
     return true;
    }
    return false;
   }
