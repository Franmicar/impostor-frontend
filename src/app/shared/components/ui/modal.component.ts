import { ChangeDetectionStrategy, Component, input, output, effect, OnDestroy } from '@angular/core';
...
export class ModalComponent implements OnDestroy {
  ...
  title = input<string>('');
  icon = input<'info' | 'error' | 'success' | 'warning' | null>(null);
  preventCloseOutside = input<boolean>(false);
  maxWidthClass = input<string>('max-w-sm');
  bodyClass = input<string>('max-h-[60vh]');
